    ...
    public function run($params)
    {
        ...
        if(is_array($symbols) && !empty($symbols)){
            foreach($symbols as $key=>$val){
                ...
                $row = $this->select('id,symbol,tradeable')->from('symbols')->where('symbol', $cols['symbol'])->row();
                if(!isset($row['id'])){
                    $this->db->insert('symbols')->cols($cols)->query();
                }else{
                    $this->db->update('symbols')->setCols($cols)
                            ->where('id', $row['id'])->where('symbol', $cols['symbol'])->query();
                }
                if(isset($row['tradeable']) && $row['tradeable'] == 1){
                    $this->gset('symbol_'.$cols['symbol'], $cols);
                }
            }
            unset($symbols, $key, $val, $data, $cols);
        }
        return true;
    }
}