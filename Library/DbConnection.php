...
class DbConnection 
{
    ...
    /**
     * 增加order by
     * @param string $col The column and direction to order by.
     * @return self
     */
    ...
    protected function addWhere($andor, $conditions)
    {
        if(is_array($conditions) && count($conditions)==2 && 
            is_string($conditions[0]) && !strpos($conditions[0], '?') && 
            is_scalar($conditions[1])
        ){
            if(in_array(substr($conditions[0], -1), ['>','<','='])){
    ...
    protected function buildValuesForUpdate()
    {
        ...
        foreach ($this->col_values as $col => $value) {
            if(isset($value['op']) && isset($value['val']) && in_array($value['op'], array('+','-','*','/'))){
                $op = $value['op'];
                $val = $value['val'];
    ...
    */
    ...
    public function query($query = '',$params = null, $fetchmode = \PDO::FETCH_ASSOC)
    {
        ...
        $exe_result = $this->execute($query,$params);
        ...
        elseif ( $statement === 'update' || $statement === 'delete' ) {
            return $this->sQuery->rowCount() ? $this->sQuery->rowCount() : $exe_result;
        }
        elseif( $statement === 'insert' ){
            if( $this->sQuery->rowCount() > 0 ){
                $lastId = intval($this->lastInsertId());
                return  $lastId === 0 ? $this->sQuery->rowCount() : $lastId;
            }
        }
        else {
            return NULL;
    ...
    {
        if(in_array($attr, array(\PDO::ATTR_EMULATE_PREPARES,\PDO::ATTR_ERRMODE,\PDO::ATTR_TIMEOUT))){
